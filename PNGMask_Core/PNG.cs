...
namespace PNGMask_Core
{
    public sealed class PNG
    {
        ...
        {
            if (data.CanTimeout) throw new PNGMask_CoreException("Stream must not be able to time-out");
            if (!data.CanRead) throw new PNGMask_CoreException("Stream must be readable");
            if (!data.CanSeek) throw new PNGMask_CoreException("Stream must be seekable");

            if (data.Length < MAGIC_NUMBER.Length) throw new PNGMask_CoreException("File is smaller than magic number");

            byte[] buffer = new byte[MAGIC_NUMBER.Length];
            if (data.Read(buffer, 0, buffer.Length) < buffer.Length) throw new PNGMask_CoreException("Could not read magic number");

            for (int i = 0; i < buffer.Length; i++)
                if (buffer[i] != MAGIC_NUMBER[i]) throw new PNGMask_CoreException("Incorrect magic number, this file is not a PNG.");
            ...
            do
            {
                ...
                if (first)
                {
                    if (name != "IHDR") throw new PNGMask_CoreException("First chunk must be IHDR, PNG is corrupt.");
                    if (length < 8) throw new PNGMask_CoreException("IHDR chunk is too small, PNG is corrupt");
                    first = false;
                }
        ...
        static void ReadData(Stream data, ref byte[] buffer)
        {
            if (data.Read(buffer, 0, buffer.Length) < buffer.Length)
                throw new PNGMask_CoreException("Unexpected EOF, PNG corrupt?");
        }

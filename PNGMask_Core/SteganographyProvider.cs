            ...
            switch (name)
            {
                ...
                case "IDX":
                    using (MemoryStream stream = new MemoryStream(ndata))
                    {
                        ...
                        do
                        {
                            if (stream.Read(ibuffer, 0, 4) < 4) throw new PNGMask_CoreException("IDX chunk is corrupt");
                            uint len = BitConverter.ToUInt32(ibuffer, 0);

                            if (stream.Read(sbuffer, 0, 3) < 3) throw new PNGMask_CoreException("IDX chunk is corrupt");
                            string chunkname = Encoding.ASCII.GetString(sbuffer);

                            byte[] dbuffer = new byte[len];
                            if (stream.Read(dbuffer, 0, (int)len) < (int)len) throw new PNGMask_CoreException("IDX chunk is corrupt");
                            using (MemoryStream ms = new MemoryStream(dbuffer))
                            {
                                if (chunkname == "IDI")
                                {
                                    do
                                    {
                                        if (ms.Read(ibuffer, 0, 4) < 4) throw new PNGMask_CoreException("IDI chunk is corrupt");
                                        int clen = BitConverter.ToInt32(ibuffer, 0);

                                        byte[] imgbuffer = new byte[clen];
                                        if (ms.Read(imgbuffer, 0, clen) < clen) throw new PNGMask_CoreException("IDI chunk is corrupt");
                                ...
                                else if (chunkname == "IDL")
                                {
                                    do
                                    {
                                        if (ms.Read(ibuffer, 0, 4) < 4) throw new PNGMask_CoreException("IDL chunk is corrupt");
                                        ...
                                        if (ms.Read(strbuffer, 0, slen) < slen) throw new PNGMask_CoreException("IDL chunk is corrupt");
